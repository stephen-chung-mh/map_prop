...
def plot(curves, names, mv_n=100, end_n=1000, xlabel="Episodes", ylabel="Running Average Return", ylim=None, loc=4):  
  plt.figure(figsize=(5, 3.5), dpi=80) 
  colors = ['red', 'blue', 'green', 'grey','orange']
  for i, m in enumerate(names.keys()):    
    v = np.array([mv(i[:end_n], mv_n) for i in curves[m][0]])
    v = np.mean(v, axis=0)
    r_std = np.std(v, axis=0)/np.sqrt(len(curves[m][0]))
    v = np.concatenate([np.full([mv_n-1,],np.nan), v])
    k = names[m]
    ax = plt.gca()         
    ax.plot(np.arange(len(v)), v, label=k, color=colors[i])
    ax.fill_between(np.arange(len(v)), v-r_std, v+r_std, label=None, alpha=0.2, color=colors[i])
  plt.xlabel(xlabel)
  plt.ylabel(ylabel)      
